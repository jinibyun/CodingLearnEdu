...
/*------- Transaction Types -------*/
-- 1. Auto Commit Transaction (default) : 자동으로 Error 를 발생해서, 데이터 변경을 취소. 하지만, 두 가지 이상의 DML 에 대해서 모든 변경을 실행/취소 할 수 있는 것은 아니다.
...
COMMIT TRANSACTION T1 -- 제일 바깥에 있는 transaction 은 inner transaction commit 사항도 기억하고 함께 물리적으로 DB 에 적용한다.
...
-- Trasaction 시작시 각 각의 DML 구문의 적당한 자리에 Partial Rollback 이 가능하도록 Save Transaction 을 삽입하여 일종의 Point 정보를 둔다.
BEGIN TRANSACTION 
...
-- 이 후에 
Rollback Transaction SavePoint2; -- SavePoint2 를 찾아 그 시점부터 시작되는 모든 DML 은 rollback 한다는 의미
