...
-- test
EXECUTE uspFindProducts 900, 1100;
...
ALTER PROCEDURE uspFindProductByModel (
    @model_year SMALLINT
	--, -- input 용
    --@product_count INT OUTPUT -- output 용 파라미터 (return 값을 받기위한 파라미터)
) AS
BEGIN
    SELECT 
        product_name,
    ...
    --SELECT @product_count = @@ROWCOUNT; -- @@ 의 전역 변수 (SQL 서버가 자체적으로 사용하는 변수). 영향 받은 레코드 수를 SQL 서버가 저장하게 된다.
...
-- test with return
DECLARE @count INT;
exec @count = uspFindProductByModel 2018
select @count
...
-- test
exec uspGetSalesAmountByYearAndName 2017, 'Lay'
